...
void ConfigManager::initDefaultConfig()
{
    ...
    // 路径配置，统一放在WeBot目录下
    logPath = weBotPath + "/logs/";
    questionLibraryPath = weBotPath + "/Questions.txt";
    keywordLibraryPath = weBotPath + "/Keywords.txt";
    ...
    // 初始化默认图标路径，统一使用 WeBot/templates
    QString templatePath = weBotPath + "/templates/";
    ...
    iconPaths["mindspark_small"] = templatePath + "MindSpark_small.png";
    ...
    // 文件路径管理
    configFilePath = weBotPath + "/config.ini";
    questionFilePath = weBotPath + "/Questions.txt";
    keywordFilePath = weBotPath + "/Keywords.txt";
    logFilePath = weBotPath + "/logs/";
    templateFilePath = weBotPath + "/templates/";
...
bool ConfigManager::loadConfig()
{
    try {
        QString documentsPath = QStandardPaths::writableLocation(QStandardPaths::DocumentsLocation);
        QString weBotPath = documentsPath + "/WeBot";
        QString configPath = weBotPath + "/config.ini";
        ...
        // 读取路径配置，确保路径使用正确的基准路径
        QString defaultConfigPath = weBotPath + "/config.ini";
        QString defaultLogPath = weBotPath + "/logs/";
        QString defaultQuestionPath = weBotPath + "/Questions.txt";
        QString defaultKeywordPath = weBotPath + "/Keywords.txt";
        QString defaultTemplatePath = weBotPath + "/templates/";
        
        configFilePath = settings.value("Paths/ConfigFilePath", defaultConfigPath).toString();
        questionFilePath = settings.value("Paths/QuestionFilePath", defaultQuestionPath).toString();
        keywordFilePath = settings.value("Paths/KeywordFilePath", defaultKeywordPath).toString();
        logFilePath = settings.value("Paths/LogFilePath", defaultLogPath).toString();
        templateFilePath = settings.value("Paths/TemplateFilePath", defaultTemplatePath).toString();
        
        logPath = settings.value("Paths/LogPath", defaultLogPath).toString();
        questionLibraryPath = settings.value("Paths/QuestionLibraryPath", defaultQuestionPath).toString();
        keywordLibraryPath = settings.value("Paths/KeywordLibraryPath", defaultKeywordPath).toString();
...
bool ConfigManager::saveConfig()
{
    try {
        ...
        settings.setValue("Paths/QuestionFilePath", questionFilePath);
        settings.setValue("Paths/KeywordFilePath", keywordFilePath);
        settings.setValue("Paths/LogFilePath", logFilePath);
        settings.setValue("Paths/TemplateFilePath", templateFilePath);
...
// 文件路径管理方法
QString ConfigManager::getConfigFilePath() const { return configFilePath; }
void ConfigManager::setConfigFilePath(const QString &path) { 
    configFilePath = path; 
    emit configChanged(); 
}

QString ConfigManager::getQuestionFilePath() const { return questionFilePath; }
void ConfigManager::setQuestionFilePath(const QString &path) { 
    questionFilePath = path; 
    emit configChanged(); 
}

QString ConfigManager::getKeywordFilePath() const { return keywordFilePath; }
void ConfigManager::setKeywordFilePath(const QString &path) { 
    keywordFilePath = path; 
    emit configChanged(); 
}

QString ConfigManager::getLogFilePath() const { return logFilePath; }
void ConfigManager::setLogFilePath(const QString &path) { 
    logFilePath = path; 
    emit configChanged(); 
}

QString ConfigManager::getTemplateFilePath() const { return templateFilePath; }
void ConfigManager::setTemplateFilePath(const QString &path) { 
