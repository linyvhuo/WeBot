    ...
    bool Automator::performQuestionAnswer(const QString& question)
{
    ...
    // 使用传入的问题
     ...
     // 检查是否请求停止
     if (m_stopRequested) {
         recordLog("[INFO] 收到停止请求，退出performQuestionAnswer");
         return false;
     }
...
bool Automator::waitForAnswerCompletion(HWND hwnd)
{
    ...
    // 简化逻辑：等待固定时间后直接返回成功
    const int totalWaitTime = answerTimeout * 1000;
    int elapsedTime = 0;
    const int checkInterval = 500; // 每500毫秒检查一次
    
    while (elapsedTime < totalWaitTime && !m_stopRequested) {
        // 等待检查间隔，期间处理事件
        QThread::msleep(checkInterval);
        QCoreApplication::processEvents();
    ...
    // 检查是否请求停止
    if (m_stopRequested) {
        recordLog("[INFO] 等待回答过程中收到停止请求");
        recordLog("[DEBUG] waitForAnswerCompletion函数执行完成，返回false");
        return false;
    }
    
    // 无论是否检测到回答，只要等待固定时间就返回成功
    recordLog(QString("[INFO] 等待回答 %1 秒完成，返回成功").arg(answerTimeout));
    recordLog("[DEBUG] waitForAnswerCompletion函数执行完成，返回true");
    return true;
}
